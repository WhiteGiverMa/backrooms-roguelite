...
func configure(w: float, h: float, floor_col: Color, wall_col: Color) -> void:
	room_half_w = w / 2.0
	room_half_h = h / 2.0
...
func spawn_obstacles() -> void:
	...
	for i in count:
		...
		pillar.offset_left = -pw / 2.0
		pillar.offset_top = -ph / 2.0
		pillar.offset_right = pw / 2.0
		pillar.offset_bottom = ph / 2.0
		pillar.color = wall_top.color.darkened(0.3)
		pillar.position = Vector2(randf_range(-room_half_w * 0.6, room_half_w * 0.6), randf_range(-room_half_h * 0.6, room_half_h * 0.6))
		add_child(pillar)
		...
		pillar_body.position = pillar.position
